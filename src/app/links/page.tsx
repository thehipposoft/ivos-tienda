...
const FADE_UP_DELAYS = ["[animation-delay:150ms]", "[animation-delay:250ms]", "[animation-delay:350ms]"];
    ...
    <main className="relative flex min-h-screen w-full justify-center overflow-hidden">
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center px-6 pb-6 pt-[3vh] lg:pt-[1vh]">
        <Image
          ...
          height={LOGO_HEIGHT}
          priority
          className="opacity-0 motion-reduce:opacity-100 animate-[links-fade-up_0.6s_ease-out_forwards] motion-reduce:animate-none"
        />
        ...
        <ul className="flex w-full flex-col gap-4">
          {LINKS.map(({ label, href, icon }, index) => (
            <li
              key={label}
              className={`opacity-0 motion-reduce:opacity-100 animate-[links-fade-up_0.6s_ease-out_forwards] motion-reduce:animate-none ${FADE_UP_DELAYS[index]}`}
            >
              <Link
                href={href}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-4 rounded-xl bg-white px-6 py-6 font-semibold text-brand-blue shadow-lg transition-[background-color,color,transform,box-shadow] duration-300 ease-out hover:-translate-y-0.5 hover:bg-brand-blue hover:text-white hover:shadow-xl"
              >
        ...
        <div className="mt-2 flex w-full flex-col items-center gap-3">
          <div className="w-full overflow-hidden rounded-2xl opacity-0 motion-reduce:opacity-100 shadow-lg animate-[links-fade-up_0.6s_ease-out_forwards] motion-reduce:animate-none [animation-delay:450ms]">
            <iframe
              src={MAP_EMBED_URL}
              width="350"
              height="200"
          ...
          <Link
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 font-medium text-brand-blue underline opacity-0 motion-reduce:opacity-100 transition-colors duration-300 ease-out animate-[links-fade-up_0.6s_ease-out_forwards] motion-reduce:animate-none [animation-delay:550ms] hover:text-brand-red"
          >
